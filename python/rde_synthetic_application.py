...
def build_initial_state(data, handler, idynEstimator):
    ...
    joint_list_dataset = joint_list_config
    ...
    x0.ds = data['sim']['ds'][0, joints_idx_in_dataset]

    s = data['sim']['s'][0, joints_idx_in_dataset]
    dds = data['sim']['dds'][0, joints_idx_in_dataset]

    x0.tau_m = data['sim']['tau_m'][0, joints_idx_in_dataset]

    x0.tau_F = data['sim']['tau_F'][0, joints_idx_in_dataset]
...
def build_estimator_input(data, index, handler, ft_offsets, joints_idx_in_dataset):
    ...
    input_rde.jointPositions = data['sim']['s'][index, joints_idx_in_dataset]
    input_rde.jointVelocities = data['sim']['ds'][index, joints_idx_in_dataset]

    input_rde.motorCurrents = data['sim']['i_m'][index, joints_idx_in_dataset]

    ft_list = handler.get_group("FT").get_parameter_vector_string("names")
    temp1 = dict()
    for ft in ft_list:
        ft_temp = data['sim']['expected_fts'][ft][index]
    ...
    acc_names = handler.get_group("ACCELEROMETER").get_parameter_vector_string("names")
    temp2 = dict()
    i = 0
    for acc in acc_frames:
        temp2[acc_names[i]] = data['sim']['accelerometers'][acc+'_acc'][index]
        i = i + 1
    input_rde.linearAccelerations = temp2

    gyro_frames = handler.get_group("GYROSCOPE").get_parameter_vector_string("frames")
    gyro_names = handler.get_group("GYROSCOPE").get_parameter_vector_string("names")
    temp3 = dict()
    i = 0
    for gyro in gyro_frames:
        temp3[gyro_names[i]] = data['sim']['gyros'][gyro+'_gyro'][index]
        i = i + 1
...
def parse_output(output, handler):
    ...
    for index in range(len(output)):
        ...
        # print(output[index].tau_F)

        rde['tau_j'].append(output[index].tau_m - output[index].tau_F)

        for ft in ft_list:
            rde['ft'][ft].append(output[index].ftWrenches[ft])
            # rde['ft_bias'][ft].append(output[index].ftWrenchesBiases[ft+'_bias'])

        for acc in acc_list:
            rde['acc_bias'][acc].append(output[index].accelerometerBiases[acc+'_bias'])

        for gyro in gyro_list:
            rde['gyro_bias'][gyro].append(output[index].gyroscopeBiases[gyro+'_bias'])
    ...
    #     rde['ft_bias'][ft] = np.array(rde['ft_bias'][ft])
    for acc in acc_list:
        rde['acc_bias'][acc] = np.array(rde['acc_bias'][acc])
    for gyro in gyro_list:
        rde['gyro_bias'][gyro] = np.array(rde['gyro_bias'][gyro])
    for contact in contact_list:
...
def main():
    ...
    [initial_state, ft_offset, joints_idx_in_dataset] = build_initial_state(data, param_handler.get_group("MODEL"), idynEstimator)
    ...
    pbar = ProgressBar()
    for index in pbar(range(data['sim']['s'].shape[0])):
    # for index in pbar(range(10)):
        index1 = index
    ...
    with open('output_estimation/' + robot_name + '/workspace_' + timestr + '.pickle', 'wb') as handle:
